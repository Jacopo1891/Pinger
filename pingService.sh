...
function sendTelegramMessage() {
        echo $( curl --write-out %{http_code} --silent --output /dev/null -X POST -s --data "chat_id=${chatId}" --data "disable_web_page_preview=true"  --data "text=$1" --connect-timeout 30 --max-time 45  "https://api.telegram.org/bo    t${botToken}/sendMessage" )
    }
